...
object ImageNetApp {
  ...
  def main(args: Array[String]) {
    ...
    var trainRDD = loader.apply(sc, "ILSVRC2012_img_train/train.000", "train.txt", false, fullHeight, fullWidth)
    logger.log("loading test data")
    val testRDD = loader.apply(sc, "ILSVRC2012_img_val/val.00", "val.txt", false, fullHeight, fullWidth)
