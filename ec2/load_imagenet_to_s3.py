...
parser.add_argument("num_train_chunks", help="Number of train .tar files generated", default=1000)
parser.add_argument("num_val_chunks", help="Number of val .tar files generated", default=50)
args = parser.parse_args()

"""Change this function if you want to upload to HDFS or local storage"""
def upload_file(targetname, stream):
  print "uploading", targetname, "to bucket", args.s3_bucket
...
# TODO: resizing not done here yet
def resize_and_add_image(next_file, file_name, content, resize=False):
  stream = io.BytesIO(content)
  if resize:
    raise NotImplemented("implement resizing")
  tarinfo = tarfile.TarInfo(name=file_name)
  tarinfo.size = len(content)
  next_file.addfile(tarinfo, fileobj=stream)
...
def process_val_files(val_tar_file, val_label_file, num_chunks):
  ...
  for i, chunk in enumerate(chunks):
    ...
    for file_name, label in chunk:
      content = val_file.extractfile(file_name).fileobj.read()
      resize_and_add_image(next_file, file_name, content)
    output.seek(0)
    upload_file("test/val." + str(i).zfill(3) + ".tar", output)
...
def process_train_files(train_tar_file, train_label_file, num_chunks):
  ...
  for i, chunk in enumerate(chunks):
    ...
    for file_name, label in chunk:
      ...
      content = file_handle.extractfile(file_handle.next()).fileobj.read()
      resize_and_add_image(next_file, file_name, content)
    output.seek(0)
